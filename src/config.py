...
SUPPORTED_WATER_MODELS: dict[str, tuple[str, str]] = {
    ...
    "opc": ("amber14/opc.xml", "tip4pew"),  # OPC uses TIP4P-Ew geometry for solvent placement
...
class UmbrellaConfig:
    ...
    save_interval_ps: float = 1.0
    pre_position_velocity_nm_per_ps: float = 0.01  # nm/ps, steered pre-positioning pulling velocity
    pre_position_spring_constant_kj_mol_nm2: float = 1000.0  # kJ/mol/nm^2, pre-positioning spring constant
    equilibration_duration_ps: float = 200.0  # ps, biased equilibration per window before production
    detect_equilibration: bool = True  # apply Chodera automated equilibration detection to production timeseries
...
class MetadynamicsConfig:
    ...
    Default values follow Barducci et al. (2008) recommendations for
    protein-ligand unbinding along a COM-distance collective variable.
    """
...
class AMOEBAConfig:
    ...
    Parameters follow Ponder et al. (2010) defaults for protein simulations.
    """
...
class MLPotentialConfig:
    ...
    Supports ANI-2x (Devereux et al., 2020) and MACE (Batatia et al., 2022).
    """
...
@dataclass(frozen=True)
class QMMMConfig:
    """QM/MM hybrid potential parameters (stub — not yet implemented).

    Future implementation will partition the system into a QM region
    (catalytic triad + P1 residue, treated at DFT level) and an MM
    region (remainder, treated with classical force field).
    """
...
class FiniteSizeCorrectionConfig:
    ...
    Default values follow Hünenberger and McCammon (1999) and
    Rocklin et al. (2013) for aqueous protein systems at 310 K.
    """
...
def _build_dataclass_from_dict(dataclass_type: type, overrides: dict) -> object:
    ...
    Args:
        dataclass_type: The target frozen dataclass type.
        overrides: Dictionary of field_name -> value overrides from YAML.

    Returns:
        An instance of dataclass_type with overrides applied.

    Raises:
        TypeError: If an unrecognized field name is provided.
    """
    ...
    for field in dataclass_fields(dataclass_type):
        ...
        if value is None:
            ...
            continue
        # Coerce numeric types based on the default value's type
        default_val = field.default
        if default_val is not dataclass_fields.__class__:  # has a real default
...
def _apply_env_overrides(section_name: str, config_instance: object) -> object:
    ...
    Environment variable names follow the pattern:
        SPINK7_<SECTION>_<FIELD> (all uppercase)

    For example:
        SPINK7_PRODUCTION_DURATION_NS=500.0

    Args:
        section_name: The configuration section name (e.g., "production").
        config_instance: The frozen dataclass instance to override.

    Returns:
        A new dataclass instance with environment overrides applied.
    """
    ...
    for field in dataclass_fields(config_instance):
        ...
        if env_value is not None:
            ...
            if isinstance(default_val, float):
                ...
                # Optional fields (e.g., random_seed: int | None = None)
...
def load_config(config_path: str | Path | None = None) -> dict[str, object]:
    ...
    Missing sections or fields fall back to dataclass defaults. The returned
    dictionary maps section names to populated frozen dataclass instances.

    Args:
        config_path: Path to a YAML configuration file. If None, returns
            all-default configurations.

    Returns:
        dict mapping section names ("system", "minimization", etc.) to
        their corresponding dataclass instances.

    Raises:
        FileNotFoundError: If config_path does not exist.
        TypeError: If an unrecognized field name is present.
    """
