...
def _create_amoeba_system(
    ...
    # Generalized Kirkwood models (_gk.xml) are implicit solvent and require NoCutoff
...
def validate_amoeba_dipole_convergence(
    ...
    Checks that the potential energy is finite (indicating the self-consistent
    induced dipole iteration converged) and that the energy magnitude is within
    physically reasonable bounds.

    Args:
        simulation: OpenMM Simulation with an AMOEBA system.
        max_dipole_norm: Maximum acceptable per-atom dipole magnitude in Debye.
            Values above this indicate likely convergence failure.

    Returns:
        True if dipoles appear converged, False otherwise.
    """
    import numpy as np
...
def compare_force_field_energies(
    ...
    Evaluates the potential energy at the given positions using the AMBER
    baseline and a specified alternative (AMOEBA or ML potential), enabling
    quantitative comparison of energy surfaces.

    Args:
        topology: OpenMM Topology object.
        positions: Atomic positions with units.
        system_config_amber: Config for AMBER baseline (force_field_family='amber').
        system_config_alt: Config for alternative force field.
        amoeba_config: Required if alternative is AMOEBA.
        ml_config: Required if alternative is ML potential.

    Returns:
        Dictionary with 'amber_energy_kj_mol', 'alt_energy_kj_mol',
        'difference_kj_mol', and 'alt_family'.
    """
    import numpy as np
